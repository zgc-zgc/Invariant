...
const InvariantDiscoveryOrchestrator_1 = require("./core/InvariantDiscoveryOrchestrator");
const BatchAnalysisOrchestrator_1 = require("./core/BatchAnalysisOrchestrator");
...
program
    ...
    try {
        ...
        // 启用AI通信记录（如果指定）
        if (options.logAi) {
            AICommunicationLogger_1.aiLogger.enable();
        ...
        if (!contractFile) {
            ...
            // 使用批量分析
            const batchOrchestrator = new BatchAnalysisOrchestrator_1.BatchAnalysisOrchestrator(options.materials);
        ...
        const contractCode = await fs_1.promises.readFile(contractFile, 'utf-8');
        if (options.verbose) {
            logger.info('System', `📊 合约代码长度: ${contractCode.length} 字符`);
        }
        // 初始化orchestrator
        const orchestrator = new InvariantDiscoveryOrchestrator_1.InvariantDiscoveryOrchestrator();
        // 开始发现流程
        logger.info('System', '🚀 启动不变量发现流程');
        const result = await orchestrator.discoverInvariants(contractCode, options.config);
        // 输出结果
        if (options.output) {
            await fs_1.promises.writeFile(options.output, JSON.stringify(result, null, 2));
        ...
        // 关闭AI通信记录
        if (options.logAi && AICommunicationLogger_1.aiLogger.isLoggingEnabled()) {
            logger.info('System', `\n📝 AI通信记录已保存到: ${AICommunicationLogger_1.aiLogger.getLogFilePath()}`);
            AICommunicationLogger_1.aiLogger.disable();
        }
    }
    catch (error) {
        const logger = Logger_1.Logger.getInstance();
        ...
        // 确保关闭AI通信记录
        if (AICommunicationLogger_1.aiLogger.isLoggingEnabled()) {
            AICommunicationLogger_1.aiLogger.disable();
        }
        process.exit(1);
    }
...
program
    ...
    .option('--verbose', 'Enable verbose logging', false)
    .option('--log-ai', 'Enable AI communication logging (saves prompts and responses to file)', false)
    .action(async (materialsConfig, options) => {
    try {
        const logLevel = options.verbose ? Logger_1.LogLevel.DEBUG : Logger_1.LogLevel.INFO;
        Logger_1.Logger.configure({ level: logLevel, showTimestamp: true, showModule: true, writeToFile: true });
        const logger = Logger_1.Logger.getInstance();
        // 启用AI通信记录（如果指定）
        if (options.logAi) {
            AICommunicationLogger_1.aiLogger.enable();
        ...
        // 初始化批量分析orchestrator
        const batchOrchestrator = new BatchAnalysisOrchestrator_1.BatchAnalysisOrchestrator(materialsConfig);
        if (options.resume) {
...
program
    ...
    .action(async (contractFile, prompts, options) => {
    try {
        const logger = Logger_1.Logger.getInstance();
...
`;
    ...
    if (!await fileExists(envPath)) {
        await fs_1.promises.writeFile(envPath, exampleEnv);
